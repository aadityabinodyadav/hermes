...
import (
	...
	"github.com/aadityabinodyadav/hermes/proto"
)
...
func (n *HermesNode) startGRPC() error {
	...
	n.grpcServer.RegisterKVService(&kvServiceHandler{node: n})
	n.grpcServer.RegisterRaftService(&raftServiceHandler{node: n})
	n.grpcServer.RegisterMembershipService(&membershipServiceHandler{node: n})
...
func (n *HermesNode) startConsistency() {
	...
	n.readIndex = consistency.NewReadIndexTracker(
		n.config.NodeID,
		func() {
			// In production: send heartbeat to all followers
			// Here: tick Raft (simplified)
		},
	)
}
...
// Service handlers (satisfy proto interfaces)
type kvServiceHandler struct {
	proto.UnimplementedHermesKVServer
	node *HermesNode
}
